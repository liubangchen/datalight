...
DEFINE_int32(system_memory_gb, 37, "System memory available for Presto Server in Gb.");

DEFINE_bool(enable_serialized_page_checksum, true, "Enable use of CRC in exchange");
...
static protocol::Duration getUptime(std::chrono::steady_clock::time_point & start)
{
...
std::string getLocalIp()
{
    ...
    VELOX_FAIL("Could not infer Node IP. Please specify node.ip in the node.properties file.");
}
...
void enableChecksum()
{
    ...
    {
        const protocol::ServerInfo serverInfo{{"381"}, "production", false, false, std::make_shared<protocol::Duration>(getUptime(start_))};
    ...
    void TrinoServer::run()
    {
        ...
        httpServer_->registerGet(
            ...
            [server = this](
                proxygen::HTTPMessage * /*message*/,
                const std::vector<std::unique_ptr<folly::IOBuf>> & /*body*/,
                proxygen::ResponseHandler * downstream) { server->reportServerInfo(downstream); });
        httpServer_->registerGet(
            "/v1/info/state",
            [server = this](
                proxygen::HTTPMessage * /*message*/,
        ...
        httpServer_->registerGet(
            ...
            [server = this](
                proxygen::HTTPMessage * /*message*/,
                const std::vector<std::unique_ptr<folly::IOBuf>> & /*body*/,
                proxygen::ResponseHandler * downstream) { server->reportNodeStatus(downstream); });
        ...
        taskManager_ = std::make_unique<TaskManager>(systemConfig->values(), nodeConfig->values());
        ...
        PeriodicTaskManager periodicTaskManager(driverCPUExecutor(), httpServer_->getExecutor());
