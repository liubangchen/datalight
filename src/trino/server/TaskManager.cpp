...
namespace datalight::server
{
    ...
    namespace
    {
        ...
        {
            handlerState->runOnFinalization([promiseHolder]() mutable { promiseHolder.reset(); });
        }
    }
    ...
    TaskManager::TaskManager(
        ...
        : bufferManager_(velox::exec::PartitionedOutputBufferManager::getInstance().lock())
        , queryContextManager_(properties, nodeProperties)
        , maxDriversPerTask_(100)
        , concurrentLifespansPerTask_(50)
{
    VELOX_CHECK_NOT_NULL(bufferManager_, "invalid PartitionedOutputBufferManager");
}
...
void TaskManager::acknowledgeResults(const TaskId & taskId, const long bufferId, long token)
{
    VLOG(1) << "TaskManager::acknowledgeResults " << taskId << ", " << bufferId << ", " << token;
...
/**
   bufferManager.getData(
            ...
            [taskId = taskId, bufferId = bufferId, promiseHolder](
                std::vector<std::shared_ptr<exec::SerializedPage>> & pages, int64_t sequence) mutable
            {
                ...
                VLOG(1) << "Task " << taskId << ", buffer " << bufferId << ", sequence " << sequence << " Results size: " << bytes
                        << ", page count: " << pages.size() << ", complete: " << std::boolalpha << complete;
...
std::unique_ptr<TaskInfo> TaskManager::createOrUpdateErrorTask(const TaskId & taskId, const std::exception_ptr & exception)
{
    ...
    std::unordered_map<std::string, std::unordered_map<std::string, std::string>> && connectorConfigStrings)
{
    ...
    uint32_t concurrentLifespans;
    {
        ...
        if (not prestoTask->task && planFragment.planNode)
        {
            ...
            auto queryCtx = queryContextManager_.findOrCreateQueryCtx(taskId, std::move(configStrings), std::move(connectorConfigStrings));
            maxDrivers = queryCtx->get<int32_t>(kMaxDriversPerTask.data(), maxDriversPerTask_);
            concurrentLifespans = queryCtx->get<int32_t>(kConcurrentLifespansPerTask.data(), concurrentLifespansPerTask_);
            ...
            execTask = std::make_shared<exec::Task>(taskId, planFragment, 0, std::move(queryCtx));
    ...
    if (startTask)
    {
        ...
        {
            LOG(INFO) << "Starting task " << taskId << " with " << maxDrivers << " max drivers and " << concurrentLifespans
                      << " concurrent lifespans.";
        }
        else
        {
            LOG(INFO) << "Starting task " << taskId << " with " << maxDrivers << " max drivers.";
        }
    ...
    for (const auto & entry : resultRequests)
    {
        ...
        VLOG(1) << "Processing pending result request for task " << resultRequest->taskId << ", buffer " << resultRequest->bufferId
    ...
    {
        execTask->updateBroadcastOutputBuffers(outputBuffers.buffers.size(), outputBuffers.noMoreBufferIds);
    }
    /**
    for (const auto & source : sources)
    {
        // Add all splits from the source to the task.
        LOG(INFO) << "Adding " << source.splits.size() << " splits to " << taskId << " for " << source.planNodeId;
        ...
        for (const auto & protocolSplit : source.splits)
        {
            ...
            {
                maxSplitSequenceId = std::max(maxSplitSequenceId, protocolSplit.sequenceId);
                execTask->addSplitWithSequence(source.planNodeId, std::move(split), protocolSplit.sequenceId);
            }
        }
    ...
    **/
    ...
    {
        promiseHolder->promise.setValue(std::make_unique<protocol::TaskStatus>(info.taskStatus));
    }
    return std::make_unique<TaskInfo>(info);
}

std::unique_ptr<TaskInfo> TaskManager::deleteTask(const TaskId & taskId, bool /*abort*/)
{
    ...
    if (execTask)
    {
        ...
        prestoTask->info.stats.endTime = util::toISOTimestamp(velox::getCurrentTimeMs());
...
namespace
{
    ...
    struct ZombieTaskCounts
    {
        ...
        ZombieTaskCounts() { taskIds.reserve(numSampleTaskId); }
        ...
        void logZombieTaskStatus(const std::string & hangingClassName)
        {
            ...
            LOG(ERROR) << "There are " << numTotal << " zombie " << hangingClassName
                       ...
                       << hangingClassName << " are referenced by more than 1 owners. RUNNING[" << numRunning << "] FINISHED["
                       << numFinished << "] CANCELED[" << numCanceled << "] ABORTED[" << numAborted << "] FAILED[" << numFailed
                       << "]  Sample task IDs (shows only " << numSampleTaskId << " IDs): {" << taskIdsStr << "}";
        }
    };
...
size_t TaskManager::cleanOldTasks()
{
    ...
    if (not taskIdsToClean.empty())
    {
        ...
        LOG(INFO) << "cleanOldTasks: Cleaned " << taskIdsToClean.size() << " old task(s) in " << elapsedMs << "ms";
    }
    else if (elapsedMs > 1000)
    {
        // If we took more than 1 second to run this, something might be wrong.
        LOG(INFO) << "cleanOldTasks: Didn't clean any old task(s). Took " << elapsedMs << "ms";
    }
    ...
    //REPORT_ADD_STAT_VALUE(kCounterNumZombiePrestoTasks, zombiePrestoTaskCounts.numTotal);
    ...
    std::shared_ptr<http::CallbackRequestHandlerState> state)
{
    ...
    auto [promise, future] = folly::makePromiseContract<std::unique_ptr<protocol::TaskInfo>>();
    auto prestoTask = findOrCreateTask(taskId);
    if (!currentState || !maxWait)
    {
        // Return current TaskInfo without waiting.
        promise.setValue(std::make_unique<protocol::TaskInfo>(prestoTask->updateInfo()));
        return std::move(future).via(eventBase);
    }

    uint64_t maxWaitMicros = std::max(1.0, maxWait.value().getValue(protocol::TimeUnit::MICROSECONDS));
    protocol::TaskInfo info;
    {
        ...
        if (!prestoTask->task)
        {
            auto promiseHolder = std::make_shared<PromiseHolder<std::unique_ptr<protocol::TaskInfo>>>(std::move(promise));
            ...
            return std::move(future).via(eventBase).onTimeout(
                ...
                [prestoTask]() { return std::make_unique<protocol::TaskInfo>(prestoTask->updateInfo()); });
        }
        info = prestoTask->updateInfoLocked();
    }
    if (currentState.value() != info.taskStatus.state || isFinalState(info.taskStatus.state))
    {
    ...
    prestoTask->task->stateChangeFuture(maxWaitMicros)
        ...
        .thenValue([promiseHolder, prestoTask](bool done)
                   { promiseHolder->promise.setValue(std::make_unique<protocol::TaskInfo>(prestoTask->updateInfo())); })
        .thenError(
...
folly::Future<std::unique_ptr<Result>> TaskManager::getResults(
    ...
    std::shared_ptr<http::CallbackRequestHandlerState> state)
{
    uint64_t maxWaitMicros = std::max(1.0, maxWait.getValue(protocol::TimeUnit::MICROSECONDS));
    VLOG(1) << "TaskManager::getResults " << taskId << ", " << bufferId << ", " << token;
    auto eventBase = folly::EventBaseManager::get()->getEventBase();
    auto [promise, future] = folly::makePromiseContract<std::unique_ptr<Result>>();

    auto promiseHolder = std::make_shared<PromiseHolder<std::unique_ptr<Result>>>(std::move(promise));
    ...
    try
    {
        ...
        for (;;)
        {
            if (prestoTask->taskStarted)
            {
                ...
                {
                    getData(promiseHolder, taskId, bufferId, token, maxSize, *bufferManager_);
                }
                return std::move(future).via(eventBase).onTimeout(std::chrono::microseconds(maxWaitMicros), timeoutFn);
            }
            std::lock_guard<std::mutex> l(prestoTask->mutex);
            if (prestoTask->taskStarted)
            {
            ...
            VLOG(1) << "Queuing up result request for task " << taskId << ", buffer " << bufferId << ", sequence " << token;
...
{
    auto eventBase = folly::EventBaseManager::get()->getEventBase();
    auto [promise, future] = folly::makePromiseContract<std::unique_ptr<protocol::TaskStatus>>();
    ...
    protocol::TaskStatus status;
    {
        ...
        if (!prestoTask->task)
        {
            auto promiseHolder = std::make_shared<PromiseHolder<std::unique_ptr<protocol::TaskStatus>>>(std::move(promise));

            keepPromiseAlive(promiseHolder, state);
            prestoTask->statusRequest = folly::to_weak_ptr(promiseHolder);
            return std::move(future).via(eventBase).onTimeout(
                std::chrono::microseconds(maxWaitMicros),
                [prestoTask]() { return std::make_unique<protocol::TaskStatus>(prestoTask->updateStatus()); });
        }
    ...
    prestoTask->task->stateChangeFuture(maxWaitMicros)
        .via(eventBase)
        .thenValue([promiseHolder, prestoTask](bool done)
                   { promiseHolder->promise.setValue(std::make_unique<protocol::TaskStatus>(prestoTask->updateStatus())); })
        .thenError(
            folly::tag_t<std::exception>{},
...
void TaskManager::removeRemoteSource(const TaskId & taskId, const TaskId & remoteSourceTaskId)
{
}
...
std::shared_ptr<TrinoTask> TaskManager::findOrCreateTaskLocked(TaskMap & taskMap, const TaskId & taskId)
{
    ...
    if (it == taskMap.cend())
    {
        ...
        prestoTask->info.taskStatus.self = fmt::format("{}/v1/task/{}", baseUri_, taskId);
...
std::string TaskManager::toString() const
{
    ...
    for (const auto & pair : *taskMap)
    {
        ...
        {
            out << exec::Task::shortId(pair.first) << " no task (" << pair.first << ")" << std::endl;
        }
    }
...
DriverCountStats TaskManager::getDriverCountStats() const
{
    ...
    for (const auto & pair : *taskMap)
    {
        ...
        {
            driverCountStats.numRunningDrivers += pair.second->task->numRunningDrivers();
        }
    }
    driverCountStats.numBlockedDrivers = velox::exec::BlockingState::numBlockedDrivers();
