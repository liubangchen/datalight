...
namespace datalight::server
{
    ...
    namespace
    {
        ...
        {
            proxygen::ResponseBuilder(downstream).status(http::kHttpOk, "OK").sendWithEOM();
        }
        ...
        void sendOkResponse(proxygen::ResponseHandler * downstream, const json & body)
        {
            proxygen::ResponseBuilder(downstream)
                ...
                .header(proxygen::HTTP_HEADER_CONTENT_TYPE, http::kMimeTypeApplicationJson)
                .body(body.dump())
                .sendWithEOM();
        }

        void sendOkThriftResponse(proxygen::ResponseHandler * downstream, const std::string & body)
        {
            proxygen::ResponseBuilder(downstream)
                .status(http::kHttpOk, "OK")
                .header(proxygen::HTTP_HEADER_CONTENT_TYPE, http::kMimeTypeApplicationThrift)
                .body(body)
                .sendWithEOM();
        }

        void
        sendErrorResponse(proxygen::ResponseHandler * downstream, const std::string & error = "", int status = http::kHttpInternalServerError)
        {
            static const int kMaxStatusSize = 1024;

            proxygen::ResponseBuilder(downstream).status(status, error.substr(0, kMaxStatusSize)).body(error).sendWithEOM();
        }

        void sendTaskNotFound(proxygen::ResponseHandler * downstream, const protocol::TaskId & taskId)
        {
            sendErrorResponse(downstream, fmt::format("Task not found: {}", taskId), http::kHttpNotFound);
        }

        std::optional<protocol::TaskState> getCurrentState(proxygen::HTTPMessage * message)
        {
            ...
            json taskStateJson = headers.getSingleOrEmpty(protocol::PRESTO_CURRENT_STATE_HTTP_HEADER);
        ...
        std::optional<protocol::Duration> getMaxWait(proxygen::HTTPMessage * message)
        {
            ...
            return protocol::Duration(headers.getSingleOrEmpty(protocol::PRESTO_MAX_WAIT_HTTP_HEADER));
        }
    }
