...
namespace datalight::server
{
    ...
    namespace
    {
        ...
        protocol::ExecutionFailureInfo toPrestoError(std::exception_ptr ex)
        {
            ...
            {
                return common::VeloxToTrinoExceptionTranslator::translate(e);
            }
        ...
        **/
        void setTiming(const CpuWallTiming & timing, int64_t & count, protocol::Duration & wall, protocol::Duration & cpu)
        {
            count = timing.count;
    ...
    TrinoTask::TrinoTask(const std::string & taskId) : id(taskId)
    {
        //info.taskId = taskId;
    }

    void TrinoTask::updateHeartbeatLocked()
    {
        lastHeartbeatMs = velox::getCurrentTimeMs();
        info.lastHeartbeat = util::toISOTimestamp(lastHeartbeatMs);
    }

    uint64_t TrinoTask::timeSinceLastHeartbeatMs() const
    {
    ...
    protocol::TaskStatus TrinoTask::updateStatusLocked()
    {
        if (!taskStarted)
        {
    ...
    protocol::TaskInfo TrinoTask::updateInfoLocked()
    {
        updateStatusLocked();
        ...
        prestoTaskStats.createTime = util::toISOTimestamp(taskStats.executionStartTimeMs);
        prestoTaskStats.firstStartTime = util::toISOTimestamp(taskStats.firstSplitStartTimeMs);
        prestoTaskStats.lastStartTime = util::toISOTimestamp(taskStats.lastSplitStartTimeMs);
        prestoTaskStats.lastEndTime = util::toISOTimestamp(taskStats.executionEndTimeMs);
        ...
        **/
        ...
        {
            taskRuntimeStats["outputConsumedDelayInNanos"].addValue((taskStats.endTimeMs - taskStats.executionEndTimeMs) * 1'000'000);
...
std::string TrinoTask::taskNumbersToString(const std::array<size_t, 5> & taskNumbers)
{
    ...
    for (size_t i = 0; i < taskNumbers.size(); ++i)
    {
        if (taskNumbers[i] != 0)
        {
            folly::toAppend(fmt::format("{}={} ", taskStateNames[i], taskNumbers[i]), &str);
        }
    }
    return str;
}
