...
extern const char* const TRINO_PAGES_MIME_TYPE;

extern const char* const TRINO_CURRENT_STATE_HTTP_HEADER;
extern const char* const TRINO_MAX_WAIT_HTTP_HEADER;
extern const char* const TRINO_MAX_SIZE_HTTP_HEADER;
extern const char* const TRINO_TASK_INSTANCE_ID_HEADER;
extern const char* const TRINO_PAGE_TOKEN_HEADER;
extern const char* const TRINO_PAGE_NEXT_TOKEN_HEADER;
extern const char* const TRINO_BUFFER_COMPLETE_HEADER;
extern const char* const TRINO_TASK_FAILED;

extern const char* const TRINO_MAX_WAIT_DEFAULT;
extern const char* const TRINO_MAX_SIZE_DEFAULT;

extern const char* const TRINO_ABORT_TASK_URL_PARAM;
...
template <typename V>
struct adl_serializer<datalight::trino::protocol::Map<int, V>> {
  static void to_json(
      json& j,
      const datalight::trino::protocol::Map<int, V>& p) {
    j = json::object();
    for (auto& el : p) {
  ...
  static void from_json(
      const json& j,
      datalight::trino::protocol::Map<int, V>& p) {
    for (auto& el : j.items()) {
...
template <typename K, typename V>
struct adl_serializer<datalight::trino::protocol::Map<K, V>> {
  static void to_json(json& j, const datalight::trino::protocol::Map<K, V>& p) {
    j = json::object();
    for (auto& el : p) {
  ...
  static void from_json(
      const json& j,
      datalight::trino::protocol::Map<K, V>& p) {
    for (auto& el : j.items()) {
...
namespace datalight::trino::protocol {
enum class BucketingVersion { BUCKETING_V1 };
...
namespace datalight::trino::protocol {
enum class FunctionKind { SCALAR, AGGREGATE, WINDOW, TABLE };
...
namespace datalight::trino::protocol {
enum class ColumnType { PARTITION_KEY, REGULAR, SYNTHESIZED };
...
template <typename T>
struct adl_serializer<datalight::trino::protocol::ColumnDomain<T>> {
  static void to_json(
      json& j,
      const datalight::trino::protocol::ColumnDomain<T>& p) {
    datalight::trino::protocol::to_json_key(
  ...
  static void from_json(
      const json& j,
      datalight::trino::protocol::ColumnDomain<T>& p) {
    datalight::trino::protocol::from_json_key(
        j, "column", p.column, "ColumnDomain", "T", "column");
    datalight::trino::protocol::from_json_key(
        j, "domain", p.domain, "ColumnDomain", "Domain", "domain");
  }
...
template <typename T>
struct adl_serializer<datalight::trino::protocol::TupleDomain<T>> {
  static void to_json(
      json& j,
      const datalight::trino::protocol::TupleDomain<T>& tup) {
    datalight::trino::protocol::List<
        datalight::trino::protocol::ColumnDomain<T>>
        list;
    if (tup.domains != nullptr) {
      for (auto& el : *tup.domains) {
        datalight::trino::protocol::ColumnDomain<T> domain;
  ...
  static void from_json(
      const json& j,
      datalight::trino::protocol::TupleDomain<T>& tup) {
    if (j.count("columnDomains") != 0U) {
      std::shared_ptr<datalight::trino::protocol::
                          Map<T, datalight::trino::protocol::Domain>>
          map = std::make_shared<
              std::map<T, datalight::trino::protocol::Domain>>();

      datalight::trino::protocol::List<
          datalight::trino::protocol::ColumnDomain<T>>
          list = j.at("columnDomains");
      for (const datalight::trino::protocol::ColumnDomain<T>& value : list) {
...
struct adl_serializer<
    datalight::trino::protocol::TupleDomain<std::shared_ptr<T>>> {
  static void to_json(
      json& j,
      const datalight::trino::protocol::TupleDomain<std::shared_ptr<T>>& tup) {
    datalight::trino::protocol::List<
        datalight::trino::protocol::ColumnDomain<std::shared_ptr<T>>>
        list;
    if (tup.domains != nullptr) {
      for (auto& el : *tup.domains) {
        datalight::trino::protocol::ColumnDomain<std::shared_ptr<T>> domain;
        domain.column = el.first;
        domain.domain = el.second;
        list.push_back(domain);
      }
  ...
  static void from_json(
      const json& j,
      datalight::trino::protocol::TupleDomain<std::shared_ptr<T>>& tup) {
    if (j.count("columnDomains") != 0U) {
      auto map = std::make_shared<std::map<
          std::shared_ptr<T>,
          datalight::trino::protocol::Domain,
          datalight::trino::protocol::pointerDerefCompare<T>>>();

      datalight::trino::protocol::List<
          datalight::trino::protocol::ColumnDomain<std::shared_ptr<T>>>
          list = j.at("columnDomains");
      for (const datalight::trino::protocol::ColumnDomain<std::shared_ptr<T>>&
               value : list) {
        map->insert(
...
struct TableWriterInfo {
  int64_t pageSinkPeakMemoryUsage = {};
  Duration statisticsWallTime = {};
  Duration statisticsCpuTime = {};
  Duration validationCpuTime = {};
};
...
namespace datalight::trino::protocol {
enum class BufferType { PARTITIONED, BROADCAST, ARBITRARY, SPOOL };
...
namespace datalight::trino::protocol {
enum class ExchangeNodeScope { LOCAL, REMOTE };
...
namespace datalight::trino::protocol {
struct DeleteTarget : public WriterTarget {
  std::shared_ptr<TableHandle> handle = {};
  SchemaTableName schemaTableName = {};

  DeleteTarget() noexcept;
};
...
namespace datalight::trino::protocol {
struct FileSystemExchangeSinkInstanceHandle
    : public ExchangeSinkInstanceHandle {
...
namespace datalight::trino::protocol {
struct CreateTarget : public WriterTarget {
  OutputTableHandle handle = {};
  SchemaTableName schemaTableName = {};
  bool reportingWrittenBytesSupported = {};

  CreateTarget() noexcept;
};
...
namespace datalight::trino::protocol {
struct InsertTarget : public WriterTarget {
  InsertTableHandle handle = {};
  SchemaTableName schemaTableName = {};
  bool reportingWrittenBytesSupported = {};

  InsertTarget() noexcept;
};
...
template <typename T>
struct adl_serializer<
    datalight::trino::protocol::StatisticAggregationsDescriptor<T>> {
  static void to_json(
      json& j,
      const datalight::trino::protocol::StatisticAggregationsDescriptor<T>& p) {

  }

  static void from_json(
      const json& j,
      datalight::trino::protocol::StatisticAggregationsDescriptor<T>& p) {}
};
} // namespace nlohmann
namespace datalight::trino::protocol {
struct TableWriterNode : public PlanNode {
  ...
  std::shared_ptr<StatisticAggregationsDescriptor<Symbol>>
