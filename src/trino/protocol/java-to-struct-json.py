...
language = {
    "cpp": {
        "TypeMap": {
            r"([ ,<])(ColumnHandle|PlanNode|RowExpression|ConnectorMetadataUpdateHandle|ExchangeSourceHandle)([ ,>])": r"\1std::shared_ptr<\2>\3",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract Trino protocol data struture from Java annotations"
    )
    parser.add_argument(
        "-c", "--config", default="trino_protocol.yml", help="config file"
    )
