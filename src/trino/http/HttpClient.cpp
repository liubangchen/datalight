...
{
    HttpClient::HttpClient(folly::EventBase * eventBase, const folly::SocketAddress & address, std::chrono::milliseconds timeout)
    ...
    HttpClient::~HttpClient()
    {
        if (sessionPool_)
        {
            ...
            eventBase_->runImmediatelyOrRunInEventBaseThreadAndWait([this] { sessionPool_.reset(); });
        }
    }
    ...
    {
    public:
        ResponseHandler(const proxygen::HTTPMessage & request, const std::string & body) : request_(request), body_(body) { }

        folly::SemiFuture<std::unique_ptr<HttpResponse>> initialize(std::shared_ptr<ResponseHandler> self)
            {
                self_ = self;
        ...
        void onError(const proxygen::HTTPException & error) noexcept override { promise_.setException(error); }
    ...
    public:
        ConnectionHandler(
            ...
            : responseHandler_(responseHandler), sessionPool_(sessionPool), timer_(timer), eventBase_(eventBase), address_(address)
            {
            }
    ...
    folly::SemiFuture<std::unique_ptr<HttpResponse>> HttpClient::sendRequest(const proxygen::HTTPMessage & request, const std::string & body)
    {
        auto responseHandler = std::make_shared<ResponseHandler>(request, body);
        auto future = responseHandler->initialize(responseHandler);

        eventBase_->runInEventBaseThreadAlwaysEnqueue(
            [this, responseHandler]()
                {
                    ...
                    auto connectionHandler = new ConnectionHandler(responseHandler, sessionPool_.get(), timer_.get(), eventBase_, address_);
