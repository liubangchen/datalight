...
namespace datalight::http
{
    ...
    HttpServer::HttpServer(const folly::SocketAddress & httpAddress, int httpExecThreads)
        : httpAddress_(httpAddress)
        , httpExecThreads_(httpExecThreads)
        , httpExecutor_{
                std::make_shared<folly::IOThreadPoolExecutor>(httpExecThreads, std::make_shared<folly::NamedThreadFactory>("HTTPSrvExec"))}
    {
    }

    proxygen::RequestHandler * DispatchingRequestHandlerFactory::EndPoint::checkAndApply(
    ...
    proxygen::RequestHandler * DispatchingRequestHandlerFactory::onRequest(proxygen::RequestHandler *, proxygen::HTTPMessage * message) noexcept
    {
        auto it = endpoints_.find(message->getMethod().value());
        if (it == endpoints_.end())
        {
            return new ErrorRequestHandler(
                http::kHttpInternalServerError, fmt::format("Unsupported HTTP method: {} {}", message->getMethodString(), message->getURL()));
        }
        ...
        {
            if (auto handler = endpoint->checkAndApply(path, message, matches, args, argPtrs))
            {
                return handler;
            }
        }
        ...
        return new ErrorRequestHandler(
            http::kHttpNotFound, fmt::format("Unsupported HTTP request: {} {}", message->getMethodString(), message->getURL()));
    }

    void DispatchingRequestHandlerFactory::registerEndPoint(
        proxygen::HTTPMethod method, const std::string & pattern, const EndpointRequestHandlerFactory & endpoint)
    {
        auto it = endpoints_.find(method);
        if (it == endpoints_.end())
        {
            endpoints_[method].emplace_back(std::make_unique<EndPoint>(pattern, endpoint));
        }
        else
        {
    ...
    void HttpServer::start(std::function<void(proxygen::HTTPServer * /*server*/)> onSuccess, std::function<void(std::exception_ptr)> onError)
    {
        proxygen::HTTPServer::IPConfig cfg{httpAddress_, proxygen::HTTPServer::Protocol::HTTP};
        ...
        options.handlerFactories = proxygen::RequestHandlerChain().addThen(std::move(handlerFactory_)).build();
