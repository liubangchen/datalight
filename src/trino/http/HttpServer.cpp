...
DispatchingRequestHandlerFactory::EndPoint::checkAndApply(
    const std::string& path, proxygen::HTTPMessage* message,
    std::vector<std::string>& matches, std::vector<RE2::Arg>& args,
...
proxygen::RequestHandler* DispatchingRequestHandlerFactory::onRequest(
  ...
  if (it == endpoints_.end()) {
    return new ErrorRequestHandler(
        ...
        fmt::format("Unsupported HTTP method: {} {}",
                    message->getMethodString(), message->getURL()));
  }
  ...
  return new ErrorRequestHandler(
      http::kHttpNotFound,
      fmt::format("Unsupported HTTP request: {} {}", message->getMethodString(),
                  message->getURL()));
}

void DispatchingRequestHandlerFactory::registerEndPoint(
    proxygen::HTTPMethod method, const std::string& pattern,
...
void HttpServer::start(
  ...
  proxygen::HTTPServer::IPConfig cfg{httpAddress_,
